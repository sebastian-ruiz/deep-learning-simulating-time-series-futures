...
class GANMissing:
    ...
    def train(self, data_train, mask=None, sample_interval=50):
        ...
        for epoch in range(epochs):
            ...
            real, real_mask = self.collect_samples(data_train, 2*batch_size, self.num_c + self.num_o, mask=mask)
            real_labels = np.ones((2*batch_size, 1))

            print("real, real_mask", real.shape, real_mask.shape)

            d_loss_real = self.discriminator.train_on_batch([real, real_mask], real_labels)

            # Generate a batch of new images
            condition, condition_mask = self.collect_samples(data_train, batch_size, self.num_c, mask=mask)
            noise = np.random.normal(size=(batch_size, self.num_z, self.num_tenors))  # THIS WORKS!
            gen_imgs = self.generator.predict([condition, condition_mask, noise])
            ...
            real, real_mask = self.collect_samples(data_train, batch_size, self.num_c, mask=mask)  # THIS ALSO WORKS
            ...
            g_loss = self.combined.train_on_batch([real, real_mask, noise], real_labels)
            ...
            if epoch % sample_interval == 0:
                ...
                # plot simulation
                if self.plot:
                    generated, real_ = self.generate(data=data_train, num_simulations=100, random_data=True)
    ...
    def generate(self, data=None, mask=None, condition=None, num_simulations=1, return_real=True, remove_condition=False, random_data=False):

        if condition is None:
            if random_data:
                _condition, condition_mask = self.collect_samples(data, 1, self.num_c + self.num_o, mask=mask)
            else:
                if type(data) is list:
                    _condition = data[0][np.newaxis, -self.num_c:]
                else:
                    _condition = data[np.newaxis, -self.num_c:]
        else:
        ...
        generated = self.generator.predict([_condition[:, -self.num_c:, :], noise]) # select last self.num_c columns
    ...
    def collect_samples(self, data, batch_size, pattern_len, mask=None):

        if type(data) is list:
            _data = data[np.random.randint(len(data))]
        else:
            _data = data
        ...
        if isinstance(mask, np.ndarray):
            return np.array([_data[a:a+pattern_len, :] for a in indices]), np.array([mask[a:a+pattern_len, :] for a in indices])
        else:
            return np.array([_data[a:a + pattern_len, :] for a in indices])

    def sample_mask(self, m, n, p):
    ...
    def load_model(self, name):
        self.generator = load_model(self.config.get_filepath_gan_model(name + "_3d_simple_generator"))
        self.discriminator = load_model(self.config.get_filepath_gan_model(name + "_3d_simple_discriminator"))
        self.combined = load_model(self.config.get_filepath_gan_model(name + "_3d_simple_combined"))
