...
def plot_prediction(x, y_true, y_pred, file_name):
    ...
    for j in range(output_dim):
        ...
        past_and_true = np.hstack((past, true))

        label1 = "True values" if j==0 else "_nolegend_"
        # label2 = "True future values" if j==0 else "_nolegend_"
        label3 = "Prediction" if j==0 else "_nolegend_"

        plt.plot(range(len(past_and_true)), past_and_true, c='r', label=label1)
        # plt.plot(range(len(past), len(true)+len(past)), true, c='r', label=label2)
        plt.plot(range(len(past), len(pred)+len(past)), pred, c='b', label=label3)
    plt.legend(loc='best')
    # plt.title("Predictions v.s. true values")
    plt.savefig(config.get_filepath_img(file_name), dpi=300, bbox_inches='tight')
    plt.savefig(config.get_filepath_pgf(file_name), dpi=300, transparent=True)  # , bbox_inches='tight'
    plt.show()
    plt.close()
